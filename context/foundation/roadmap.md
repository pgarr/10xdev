...
**S-01: Guest team analysis** — The north star is the smallest end-to-end slice whose successful delivery proves the core product hypothesis — that casual players want a plain-language verdict over raw type data — placed as early as prerequisites allow because everything else only matters if this works. S-01 is both the only user story and the primary Success Criterion; the core flow ships as a single slice, not split, because a partial result (rating without coverage, or coverage without recommendations) does not prove the hypothesis.
...
What's already in place in the codebase as of 2026-05-26 (auto-researched + user-confirmed). Foundations assume these layers are present and do NOT re-scaffold them.

- **Frontend:** Partial — Astro 6 + React 19 + Tailwind CSS scaffold present; no MonRate pages, team input, or analysis UI built yet
- **Backend / API:** Absent — only starter auth endpoints (`src/pages/api/auth/`); no PokeAPI integration, no analysis logic
- **Data:** Absent — no migrations, no custom tables; `supabase/config.toml` present but unused for v1 (guest-only, stateless)
- **Auth:** Present — `src/middleware.ts` with `PROTECTED_ROUTES`; sign-in/sign-up/sign-out endpoints wired; not required for v1 guest flow
- **Deploy / infra:** Present — `wrangler.jsonc` (Cloudflare Workers), GitHub Actions CI (lint + build on push/PR), deployed live at `monrate.garlej-p.workers.dev`
- **Observability:** Absent — no logging library, no error tracking; not required for v1

## Foundations

None — all prerequisite infrastructure layers are already in place (deploy/infra present, auth scaffold present but unused for v1) and v1 requires no database (guest-only stateless flow, no persistence). PokeAPI integration is bundled into S-01 rather than split into a separate foundation: it has a single consumer and the `speed` bias favors fewer moving parts.
...
- **Risk:** The entire product value rests on this slice's correctness — wrong type data or an unexplained rating destroys trust immediately (PRD Guardrail: "wrong data destroys trust immediately"). PokeAPI is the only external dependency; it has no SLA, so graceful degradation (NFR: partial result or explanatory error, not a blank screen) is load-bearing, not optional. Sequenced first because nothing else is meaningful without it.
- **Status:** ready
...
1. **Project name** — Owner: user. Block: no. Status: resolved — "MonRate" confirmed in PRD frontmatter and `tech-stack.md`.
2. **Target scale (QPS / data volume)** — Owner: user. Block: no. Status: resolved — `qps: low`, `data_volume: small` confirmed in `tech-stack.md`.
3. **Scoring formula specifics** — How to weight base stats and define "severity" quantitatively for the analysis engine (PRD §Business Logic describes the concept but not the formula). Owner: user. Block: no — roadmap is not blocked, but `/10x-plan` should surface this as an early design decision before implementation begins.

## Parked

- **Pokémon sprite images (FR-007)** — Why parked: `speed` bias; sprites are aesthetic, not functional; PRD §Polish Socrates note: "analysis must be functionally complete before polish is added." Add after S-01 ships.
- **Named Pokémon suggestions in recommendations (FR-008)** — Why parked: `speed` bias; named suggestions require game-agnostic pick curation; PRD §Polish marks as nice-to-have with v2 game-context caveat.
