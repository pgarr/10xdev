...
- Type matchup data must be accurate — sourced live or reliably from PokeAPI; wrong data destroys trust immediately.
...
- FR-001: Guest can search for Pokémon by name using an autocomplete input and add up to 6 to their team; type and base-stat data is fetched from PokeAPI per selection. Priority: must-have
...
- A guest user sees the full analysis result within 3 seconds of submitting their team.
...
- When PokeAPI is unavailable or slow, the product degrades gracefully — a partial result or explanatory error is shown rather than a blank failure state.
- A guest user's team data is never persisted — no input submitted during a guest session is retained after the analysis is returned.
- The layout is responsive and usable on a phone-sized screen; no WCAG compliance target is set for v1.

## Business Logic

The app scores a team's type coverage completeness and defensive resilience across all member Pokémon — weighted by their base stats, adjusted for ability-modified immunities (e.g., Levitate removes Ground weakness), and grounded in each Pokémon's actual movepool rather than assumed type moves — then ranks improvement gaps by severity so the most impactful fix surfaces first.

The rule consumes four user-facing inputs per Pokémon: their type(s), base stats (HP, Attack, Defense, Sp. Atk, Sp. Def, Speed), ability, and known moves — all drawn from Pokémon species data.
...
- Save multiple named teams to a persistent database.
- Rate a saved team against a named opponent (e.g., a gym leader, a rival) to see how the matchup looks.
- Standard sign-up / sign-in flow; authentication method is TBD at stack selection.
...
- **No social features in v1**: no leaderboards, no public team gallery, no sharing. The app is a single-user analysis tool for MVP.
- **No accounts, team saving, or opponent matchup in v1**: authentication, database persistence, and gym-leader/rival matchup analysis are all explicitly v2 scope.

## Open Questions

1. **What is the project name?** — TBD by user. Block: no (PRD can be drafted with placeholder, but name should be set before the PRD is considered reviewed). Suggested options: "TeamChecker", "PokeRate", "TypeDex", "MonTeam".
2. **What are the target_scale.qps and target_scale.data_volume values?** — Inferred as `qps: low` and `data_volume: small` from the medium user scale and team data size; confirm or override at stack selection.
